...
void createGLContexts() {
  ...
  window = glfwCreateWindow(800, 800, "SoupSim", nullptr, nullptr);
  if (window == nullptr) {
...
void usageError(const char *binaryName) {
  ...
  printf("  -a     <INT>       Bowl vertices latitude direction.\n");
  printf("  -o     <INT>       Bowl vertices longitude direction.\n");
  printf("\n");
  exit(-1);
}
    ...
    if (key == FLUID) {
      ...
      if (it_height != object.end()) {
        ...
        if (it_depth != object.end()) {
            ...
            incompleteObjectError("fluid", "depth");
        }
      ...
      if (it_num_height_points != object.end()) {
        ...
        if (it_num_depth_points != object.end()) {
            ...
            incompleteObjectError("fluid", "num_depth_points");
        }
      ...
      fluid->thickness = thickness;
      

      // fluid parameters
      double viscosity, density, damping; //TODO: add more fluid params

        auto it_viscosity = object.find("viscosity");
        if (it_viscosity != object.end()) {
            viscosity = *it_viscosity;
        } else {
            incompleteObjectError("fluid", "viscosity");
        }
