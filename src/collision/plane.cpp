...
void Plane::collide(PointMass &pm) {
  ...
  if ((dot(pm.position - point, normal) >= 0) != (dot(pm.last_position - point, normal) >= 0)) { //TODO: this iffy if condition is probably causing bouncing
      ...
      Vector3D tangent_point = pm.position - proj_n_v;

      // compute correction vector for last_position
      Vector3D correction_vector = (tangent_point + normal * SURFACE_OFFSET) - pm.last_position;
