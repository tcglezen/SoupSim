...
void FluidSimulator::drawWireframe(GLShader &shader) {
    // TODO: Change this to draw dots for each particle of the fluid
  int num_structural_springs =
      2 * fluid->num_width_points * fluid->num_height_points -
      fluid->num_width_points - fluid->num_height_points;
  int num_shear_springs =
      2 * (fluid->num_width_points - 1) * (fluid->num_height_points - 1);
  int num_bending_springs = num_structural_springs - fluid->num_width_points -
                            fluid->num_height_points;
...
//                    fp->enable_bending_constraints * num_bending_springs;
  ...
  for (int i = 0; i < fluid->springs.size(); i++) {
    Spring s = fluid->springs[i];
...
void FluidSimulator::drawNormals(GLShader &shader) {
